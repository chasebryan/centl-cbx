...
def leg(a: int, p: int) -> int:
    """Legendre symbol for odd prime modulus p."""
    r = pow(a % p, (p - 1) // 2, p)
...
def classify_type2_roots() -> tuple[list[dict[str, object]], collections.Counter[str]]:
    ...
    for exps in itertools.product(*[range(2 * e + 1) for _, e in items]):
        ...
        assert d == s * b * b
        assert C(k) == s * b * c
        geom = "A" if c % b == 0 else "B" if b % c == 0 else "interior"
        counts[geom] += 1
...
def main() -> int:
    ...
    # k15 is composite, so its source character is Jacobi rather than Legendre.
    ...
    assert 41 % 31 == 10 and 10 not in {1, 5, 25}
    ...
    report = {
        ...
        "failures": 0,
        "claim": "certified prime survives k3 through k31 and first hits k35 with Type-I plus mixed Type-II geometry",
    }
